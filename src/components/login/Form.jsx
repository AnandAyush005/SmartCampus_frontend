...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // FIX: Handle HTML error response + exact backend message
    let errorMessage = 'Login failed';
    
    // Check if backend returned HTML error (status 403)
    if (error.response?.status === 403) {
      const htmlResponse = error.response?.data;
      if (htmlResponse && typeof htmlResponse === 'string') {
        // Extract error message from HTML: "You are not verified by the admin"
    ...
    // SPECIFIC "not verified" handling
    ...
    return         <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/50">
